...
class AlphaBetaAgent(Agent):
    def __init__(self, evalFn=scoreEvaluationFunction, depth=2):
        self.index = 0  # Pacman is always agent index 0
        self.evaluationFunction = evalFn  # This is now a callable function
        self.depth = int(depth)  # Set depth as an integer
    ...
    def getAction(self, gameState):
        ...
        def alphaBeta(curr_depth, agent_index, gameState, alpha, beta):
            # If all agents have played, increment depth and start from Pacman again
            if agent_index >= gameState.getNumAgents():
                agent_index = 0
                curr_depth += 1
            
            # If maximum depth is reached, return the evaluation value
            ...
            if agent_index == 0:  # Pacman's turn (Maximizing player)
                ...
                for action in gameState.getLegalActions(agent_index):
                    new_value = alphaBeta(curr_depth, agent_index + 1, gameState.generateSuccessor(agent_index, action), alpha, beta)
                    if new_value > value:
                        value = new_value
                        best_action = action
                    # Pruning condition for the maximizing player
                    if value > beta:
                        return value
                ...
                for action in gameState.getLegalActions(agent_index):
                    ...
                    if new_value < value:
                        value = new_value
                        best_action = action
                    # Pruning condition for the minimizing player
                    if value < alpha:
                        return value
                    beta = min(beta, value)
                return value
        ...
        # Loop through all possible legal actions for Pacman
        for action in gameState.getLegalActions(0):
            new_value = alphaBeta(0, 1, gameState.generateSuccessor(0, action), alpha, beta)
            if new_value > value:
                value = new_value
                best_action = action
            # Update alpha for Pacman (Maximizing player)
            alpha = max(alpha, value)
